...
namespace mltk{
    namespace classifier {
        ...
        bool IMAp<T>::train() {
            ...
            if (this->hasInitialSolution) {
                ...
                if (margin == 0) tMax = this->MAX_UP;
                    ...
                    double raio = Statistics<T>::getRadius(this->samples, -1,
            ...
            *imapFixMargin.getFlagNot1aDim() = flagNao1aDim;
            ...
            while (imapFixMargin.train()) {
                ...
                secs = stime / 1000;
                if (this->verbose)
                    cout << " " << it + 1 << "        " << this->steps << "           " << this->ctot
                         << "              " << rmargin << "            " << norm << "           " << secs << " ";
        ...
        template
        class IMAp<long long int>;

        template
        class IMAp<short int>;

        template
        class IMAp<long double>;

        template
        class IMAp<unsigned char>;

        template
        class IMAp<unsigned int>;

        template
        class IMAp<unsigned short int>;

        template
        class IMApFixedMargin<int>;

        template
        class IMApFixedMargin<double>;

        template
        class IMApFixedMargin<float>;

        template
        class IMApFixedMargin<int8_t>;

        template
        class IMApFixedMargin<char>;

        template
        class IMApFixedMargin<long long int>;

        template
        class IMApFixedMargin<short int>;

        template
        class IMApFixedMargin<long double>;

        template
        class IMApFixedMargin<unsigned char>;

        template
        class IMApFixedMargin<unsigned int>;

        template
        class IMApFixedMargin<unsigned short int>;

        template
        class IMADual<int>;

        template
        class IMADual<double>;

        template
        class IMADual<float>;

        template
        class IMADual<int8_t>;

        template
        class IMADual<char>;

        template
        class IMADual<long long int>;

        template
        class IMADual<short int>;

        template
        class IMADual<long double>;

        template
        class IMADual<unsigned char>;

        template
        class IMADual<unsigned int>;

        template
        class IMADual<unsigned short int>;
    }
}