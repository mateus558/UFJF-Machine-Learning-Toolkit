...
namespace mltk{
    namespace featselect {
        ...
        std::shared_ptr<Data<T> > RFE<T>::selectFeatures() {
            ...
            while (true) {
                ...
                if (this->leave_one_out) {
                    ...
                    cout << "LeaveOO -- Dim: " << (dim - level) << ", Margin: " << margin << ", LeaveOO: " << leave_oo
                         << ", SVs: " << svcount << endl;
                } else if (this->verbose) {
                    if (this->cv->qtde > 0 && level % this->cv->jump == 0)
                        cout << "Dim: " << (dim - level) << ", Margin: " << margin << ", SVs: " << svcount << ", Erro "
        ...
        *----------------------------------------------------------*/
        ...
        template
        class RFE<long long int>;

        template
        class RFE<short int>;

        template
        class RFE<long double>;

        template
        class RFE<unsigned char>;

        template
        class RFE<unsigned int>;

        template
        class RFE<unsigned short int>;
    }
}